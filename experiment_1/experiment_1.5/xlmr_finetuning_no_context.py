...
class Config:
    ...
    OUTPUT_DIR = 'outputs/finetuned_xlmr_no_context'
    PREDICTIONS_DIR = 'outputs/finetuned_predictions_no_context'
...
class SarcasmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # EXPERIMENT 2: Use only text (response) without context
        text_input = row['text']
...
def evaluate_and_save_predictions(model, tokenizer, dataframe, dataset_name, output_dir):
    ...
    results_df['model_name'] = 'xlmr_finetuned'
    results_df['condition'] = 'text_only'
    ...
    # Print results
    print(f"\n{'='*60}")
    print(f"Results on {dataset_name.upper()} set (TEXT ONLY - NO CONTEXT)")
    print(f"{'='*60}")
...
def main():
    print("="*60)
    print("EXPERIMENT 2: XLM-RoBERTa Fine-tuning (TEXT ONLY - NO CONTEXT)")
    print("="*60)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    print("NOTE: This experiment uses ONLY the 'text' column (response utterance)")
    print("      WITHOUT the 'context' column (preceding dialogue)\n")
    ...
    summary_path = os.path.join(Config.PREDICTIONS_DIR, 'finetuned_results_summary_no_context.csv')
    summary_df.to_csv(summary_path, index=False)
    
    print("\n" + "="*60)
    print("FINAL SUMMARY (EXPERIMENT 2: TEXT ONLY - NO CONTEXT)")
    print("="*60)
